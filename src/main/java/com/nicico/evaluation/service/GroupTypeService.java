...
public class GroupTypeService implements IGroupTypeService {
    ...
    @Transactional(readOnly = true)
    @PreAuthorize("hasAuthority('R_GROUP_TYPE')")
    public SearchDTO.SearchRs<GroupTypeDTO.GroupByInfo> searchByGroupBy(int count, int startIndex, SearchDTO.SearchRq request) {

        Pageable pageable = pageableMapper.toPageable(count, startIndex);
        List<GroupTypeByGroupByDTO.Resp> byGroupBy = repository.findAllByGroupBy(pageable.getPageNumber(), pageable.getPageSize());
        List<GroupTypeDTO.GroupByInfo> groupByDtoList = mapper.dtoInfoToGroupByDtoList(byGroupBy);
